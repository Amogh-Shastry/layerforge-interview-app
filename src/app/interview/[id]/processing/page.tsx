...
export default function ProcessingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col min-h-screen overflow-hidden bg-canvas text-ink">
      ...
      <main className="relative z-10 flex flex-col items-center justify-center flex-grow px-4 md:px-12 py-8">
        <div className="relative flex flex-col items-center mb-12">
          <div className="relative w-[280px] h-[280px]">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="brain-glow bg-surface border border-line rounded-full p-8">
                <span className="material-symbols-outlined text-accent text-[80px] material-filled">psychology</span>
              </div>
            </div>
          </div>

          <div className="mt-8 text-center space-y-2">
            <h1 className="font-display text-2xl font-bold tracking-tight text-ink">Analyzing Your Interview…</h1>
            <p className="font-sans text-ink-mute max-w-md">
        ...
        <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {baseStages.map((stage, i) => {
            ...
            return (
              <div
                key={stage.key}
                className={`glass-panel p-5 rounded-xl flex items-center gap-4 transition-all duration-300 ${active ? "border-accent/30 ring-1 ring-accent/20" : ""} ${pending ? "opacity-50" : ""}`}
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${done ? "bg-success/20" : active ? "bg-accent/10 animate-pulse" : "bg-surface-2"}`}>
                  <span className={`material-symbols-outlined text-xl ${done ? "text-success material-filled" : active ? "text-accent" : "text-ink-mute"}`}>
                    {done ? "check_circle" : stage.icon}
                  </span>
                </div>
                <div>
                  <p className={`font-mono text-sm uppercase ${done ? "text-ink" : active ? "text-accent" : "text-ink-mute"}`}>{stage.label}</p>
                  <p className="font-sans text-xs text-ink-mute">{stage.desc}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-12 w-full max-w-2xl bg-black/40 rounded-lg p-4 font-mono border border-line h-32 overflow-hidden relative">
          <div className="absolute inset-x-0 top-0 h-4 bg-gradient-to-b from-black/40 to-transparent z-10" />
          <div className="flex flex-col space-y-1 text-ink-mute text-[11px]">
            {logs.map((log, i) => (
              <div key={i} className="flex gap-2">
                <span className={i >= logs.length - 1 && progress < 100 ? "text-accent animate-pulse" : "text-success"}>
      ...
      <div className="fixed bottom-0 left-0 w-full p-8 md:px-12 z-20">
        <div className="max-w-7xl mx-auto flex flex-col gap-3">
          <div className="flex justify-between items-end">
            <span className="font-mono text-accent text-sm tracking-widest">SYSTEM_STATUS: ANALYZING_REPORT</span>
            <span className="font-mono text-ink text-sm">{Math.floor(progress)}%</span>
          </div>
