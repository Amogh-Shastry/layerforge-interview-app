        ...
        {positions.length === 0 ? (
          ...
          <div className="space-y-5">
            <div>
              ...
              <select value={positionId} onChange={(e) => setPositionId(e.target.value)} className="w-full bg-surface border border-line rounded-lg py-2.5 px-3 text-sm focus:border-accent focus:outline-none">
            ...
            <div>
              <label className="font-mono text-[11px] text-ink-mute uppercase block mb-2">Public interview link</label>
              <div className="flex items-center gap-2 bg-surface border border-line rounded-lg p-2">
                ...
                <button onClick={copy} className="px-3 py-1.5 bg-accent text-on-accent font-mono text-[11px] font-bold rounded-md">{copied ? "Copied!" : "Copy"}</button>
              </div>
              <p className="font-sans text-xs text-ink-mute mt-2">Share this with any candidate. They enter their own details and start — results appear automatically on your dashboard.</p>
            </div>

            <a href={link} target="_blank" rel="noreferrer" className="block text-center px-4 py-2.5 border border-line text-ink font-mono text-[13px] rounded-lg hover:bg-surface-2-hover transition-all">
